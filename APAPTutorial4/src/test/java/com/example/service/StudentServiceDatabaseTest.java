...
import org.mockito.BDDMockito; import org.mockito.Mock;
...
public class StudentServiceDatabaseTest {
	...
	StudentModel check = new StudentModel("1506737823", "Chanek", 3.5); checks.add(check);
	...
	//update name
	studentModel.setName("rahma");
	//make new studentmodel instance to compare
	StudentModel checkUpdate = new StudentModel("1506737823", "rahma", 3.5); 
	
	//update
	BDDMockito.given(studentService.updateStudent(studentModel)).willReturn(true);
	
	// When
		boolean testUpdate = studentService.updateStudent(studentModel);
		// Then 
		BDDMockito.then(studentMapper).should().updateStudent(checkUpdate); 
		assertThat(testUpdate, equalTo(true)); // Check if Same
	}
	...
	// When
	boolean testDelete = studentService.deleteStudent(studentModel.getNpm());
	// Then 
	BDDMockito.then(studentMapper).should().deleteStudent(check.getNpm()); 
	assertThat(testDelete, equalTo(true)); // Check if Same
	}
}
